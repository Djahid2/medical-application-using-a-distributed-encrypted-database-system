...
import loadingcercule from "../lottie/Animation - 1736145419385.json";

export default function UserData() {
    
    // zouaouiedits
    const [isOpenAction, setOpenAction] = useState(0);
    useEffect(() => {
        if (isOpenAction === 1) {
            console.log("open");
        } else {
    ...
    // 0 close 1 is open
    // zouaouiedits here the message that show in notification
    const [messagenot, setMessageNot] = useState("message");
    const notfication = useRef();
    const [isWaiting, setWaiting] = useState(false);
    const [isWaitingMain, setWaitingMain] = useState(true);
    ...
    function waiting(message) {
        ...
        setTimeout(() => {
            ...
            notfication.current.classList.add("active");
            setTimeout(() => {
                notfication.current.classList.remove("active");
    ...
    function handlRightClickRemove(e) {
        ...
        if (listOfTheDeletedObejcts.indexOf(item) === -1) {
            ...
            setListOfTheDeletedObejcts((prev) =>
                prev.filter((ele) => (ele !== item ? ele : null))
            );
        }
    }
    // //////////////////////////////////////////////////////////////////////////////////////
    ...
    const [filtervaluesreducer, dispatchfiter] = useReducer(
        reducerFitler,
        initFilter
    );
    useEffect(() => {
    ...
    useEffect(() => {
        if (isFilterValid(debouncedFilter) && !isLoadingRef.current) {
            ...
            axios
                .post(
                    "http://localhost:5000/patient/SerchPatients",
                    debouncedFilter
                )
                .then((response) => {
    ...
    const [showdata, setShowdata] = useState([
        {
            matricule: 2121,
            nom_patient: "test",
            next_appointment_date: "2024-11-05",
            diagnosis: "blabla",
            etat: true,
        },
    ]);
    ...
    function reducer(valueChageReduce, action) {
        switch (action.type) {
            case "changeAdding":
                return { ...valueChageReduce, [action.fild]: action.payload };
            case "adding":
                return { ...init };
            case "billing_information":
                return {
                    ...valueChageReduce,
                    billing_information: {
                        ...valueChageReduce.billing_information,
            ...
            case "insurance_details":
                return {
                    ...valueChageReduce,
                    insurance_details: {
                        ...valueChageReduce.insurance_details,
                        [action.filed]: action.payload,
                    },
                };
            ...
            case "emergency_contactAdd": {
                let dataaddted = JSON.parse(JSON.stringify(valueChageReduce));
                dataaddted.emergency_contact[0][`${Date.now()}`] =
                    action.payload;
                console.log(dataaddted);
                return { ...dataaddted };
            }
    ...
    //     if (
    //         e.target === passwordReq.current ||
    //         e.target === passwordReqinput.current ||
    //         e.target === theImportanteButton.current ||
    //         e.target === iCancalepasswordReq.current
    //     ) {
    //     } else {
    ...
    function handShow(data) {
        axios
            .post("http://localhost:5000/patient/findPatient", {
                data: data.matricule,
            })
            .then((response) => {
                ...
                if (Patient.next_appointment_date) {
                    const [date, timeWithZ] =
                        Patient.next_appointment_date.split("T");
                    appointmentDate = date;
                    appointmentTime = timeWithZ
                        ? timeWithZ.replace("Z", "")
                        : "";
                }

                const emergencyContact = Patient.emergency_contact
                    ? Object.entries(JSON.parse(Patient.emergency_contact)).map(
                          ([key, value]) => ({
                              [key]: value,
                          })
                      )
                ...
                const medicalHistory = Patient.medical_history
                    ? Object.entries(JSON.parse(Patient.medical_history)).map(
                          ([key, value]) => ({
                              [key]: value,
                          })
                      )
                ...
                const diagnoses = Patient.diagnoses
                    ? Object.entries(JSON.parse(Patient.diagnoses)).map(
                          ([key, value]) => ({
                              [key]: value,
                          })
                      )
                ...
                const medications = Patient.medications
                    ? Object.entries(JSON.parse(Patient.medications)).map(
                          ([key, value]) => ({
                              [key]: value,
                          })
                      )
                ...
                const treatments = Patient.treatments
                    ? Object.entries(JSON.parse(Patient.treatments)).map(
                          ([key, value]) => ({
                              [key]: value,
                          })
                      )
                ...
                const labResults = Patient.lab_results
                    ? Object.entries(JSON.parse(Patient.lab_results)).map(
                          ([key, value]) => ({
                              [key]: value,
                          })
                      )
                ...
                const symptoms = Patient.symptoms
                    ? Object.entries(JSON.parse(Patient.symptoms)).map(
                          ([key, value]) => ({
                              [key]: value,
                          })
                      )
    ...
    useEffect(() => {
        ...
        setShowdata(vid); // Mise à jour des données affichées
        console.log("showdata updated:", vid);
    }, [dataPatient]); // Dépendances

    // fonction qui permet d'ajouter le patient
    function Addpatient() {
        axios
            .post("http://localhost:5000/patient/addpatient", {
    ...
    function deletePatient() {
        ...
        axios
            .post("http://localhost:5000/patient/deletpatient", {
                passManager: passManager,
                user: User.data.info,
    ...
    useEffect(() => {
        ...
        axios
            .post("http://localhost:5000/patient/getPatient", {
                list: list,
            })
            .then((response) => {
                console.log(response.data);
                ...
                const sortedData = [...data1].sort(
                    (a, b) => b.matricule - a.matricule
                );
    ...
    function Next50Pateint() {
        const newList = list + 1; // Calculer la nouvelle valeur
        setlist(newList); // Mettre à jour l'état
        setWaitingMain(true)
        axios
            .post("http://localhost:5000/patient/getPatient", {
                list: newList, // Utiliser la nouvelle valeur ici
            })
            .then((response) => {
                console.log("50+");
                console.log(response.data);
    ...
    function uppdatepatient() {
        ...
        axios
            .post("http://localhost:5000/patient/UpdatePatients", {
    ...
    function verfAdmin() {
        axios
            .post("http://localhost:5000/auth/VerfAdmin", {
    ...
    return (
        <div className="userdata">
            {/* zouaouiedits */}
            <div ref={notfication} className="donewaiting">
                <span></span>
                {messagenot}
            </div>
            <div
            ...
            >
                ...
                <input
                    value={passManager}
                    onChange={(e) => {
                        if (actionDelAdd === 0 || actionDelAdd === 3) {
                            setpassManager(e.target.value);
                        } else {
                            setpassEditor(e.target.value);
                        }
                    }}
                    type="password"
                />
                <i
                    ...
                    ref={iCancalepasswordReq}
                    class="fa-solid fa-xmark"
                ></i>
                <button
                    ref={theImportanteButton}
                    onClick={() => {
                        if (actionDelAdd === 0) {
                            // zouaouiedits
                            deletePatient();
                ...
                >
                    {isWaiting ? <span className="loader"></span> : "Submit"}
                </button>
            </div>
            <div ref={menu} className="moreinfo">
                ...
                <div ref={addmoredatamenu} className="addmoredata">
                    <button
                        onClick={() => {
                            addmoredatamenu.current.classList.remove(
                                "activate"
                            );
                    ...
                    {
                        {
                            ...
                            medical_history: (
                                <div className="chosingDiv">
                                    ...
                                    <div>
                                        {Object.keys(
                                            valueChageReduce.medical_history[0]
                                        ).length === 0 ? null : (
                                            <table className="tableshowinfo">
                                                <tr>
                                                    <th>illness</th>
                            ...
                            medications: (
                                <div className="chosingDiv">
                                    ...
                                    <div>
                                        {Object.keys(
                                            valueChageReduce.medications[0]
                                        ).length === 0 ? null : (
                                            <table className="tableshowinfo">
                                                <tr>
                                                    <th>name</th>
                            ...
                            lab_results: (
                                <div className="chosingDiv">
                                    ...
                                    <div>
                                        {Object.keys(
                                            valueChageReduce.lab_results[0]
                                        ).length === 0 ? null : (
                                            <table className="tableshowinfo">
                                                <tr>
                                                    <th>test</th>
                ...
                <div className="intro">
                    ...
                    <button
                        disabled={isOpenAction === 1 ? true : false}
                        onClick={() => {
                            console.log("add");
                            console.log(valueChageReduce);
                            /////////////////// important ////////////////////////////
                ...
                >
                    ...
                    <input
                        ...
                        value={valueChageReduce?.patientName}
                    />
                    ...
                    <input
                        ...
                        type="date"
                        onChange={handlChangeReducer}
                        value={valueChageReduce?.date_naissance_patient}
                    />
                    ...
                    <input
                        ...
                        type="date"
                        onChange={handlChangeReducer}
                        value={valueChageReduce?.next_appointment_date}
                    />
                    ...
                    <input
                        ...
                        type="time"
                        onChange={handlChangeReducer}
                        value={valueChageReduce?.next_appointment_Time}
                    />
                    ...
                    <input
                        ...
                        type="text"
                        onChange={handlChangeReducer}
                        value={valueChageReduce?.allergies}
                    />
                    ...
                    <input
                        ...
                        type="text"
                        onChange={handlChangeReducer}
                        value={valueChageReduce?.doctor_notes}
                    />
                </form>
                <form
                    action=""
            ...
            <div className="text">
                ...
                <p>
                    from here you can change and update your all data , we hope
                    that we can satisfaite you .{" "}
                </p>
            </div>
                    ...
                    <div className="info">
                        <div className="top">
                            ...
                            <div>
                                <button
                                    onClick={() => {
                                        passwordReq.current.classList.add(
                                            "active"
                                        );
                                        setActionDelAdd(0);
                                    }}
                        ...
                        <table ref={tab}>
                            ...
                            <tbody>
                                { Array.isArray(
                                    filtervaluesreducer.searchResults.data
                                ) &&
                                    filtervaluesreducer.searchResults.data.map(
                                        (ele, i) => {
                                            console.log(
                                                "9ble afficher",
                                                filtervaluesreducer
                        ...
                        {isWaitingMain ? 
                                 <div className="animationDataloadingcercule">
                            <Lottie animationData={loadingcercule} />
                            </div> : null}
                    </div>
                ) : null}
            </div>
                        ...
                        <div className="switch">
                            ...
                            {[...Array(length_data_patient)].map((ele, i) => {
                                return (
                                    <button
                                        className={
                                            i + 1 === priv ? "active" : null
                                        }
                                        onClick={() => {
                                            handlClickButtonMove(i + 1);
                                        }}
                                    >
                                        {i + 1}
                                    </button>
                            ...
                            <button
                                onClick={() => {
                                    // here you function for pagination
                                    // zouaouiedits
                                    Next50Pateint();
                                }}
                            >
